...
public class HotelArrayList {
	...
	public void getCheapestHotel(int typeOfCustomer, int day1, int day2, int days) {
		...
		System.out.println("Hotel is : " + cityNameToPrint + " and the total " + "price would be : " + globalSum);
	}
	...
	public void getBestRatedHotel(int typeOfCustomer, int day1, int day2, int days) {
		...
		if (typeOfCustomer == 1) {
			if (diff == days) {
				...
				localSum = localSum
						+ weeks * (5 * (dumH.getHotelRateRegularWeekDay()) + 2 * (dumH.getHotelRateRegularWeekEnd()));
			}
		} else {
			if (diff == days) {
				for (int i = day1; i <= day2; i++) {
				...
				int weeks = (days - diff) / 7;
				localSum = localSum
						+ weeks * (5 * (dumH.getHotelRateRewardWeekDay()) + 2 * (dumH.getHotelRateRewardWeekEnd()));
			}
		}
		System.out.println("Hotel is : " + dumH.getHotelName() + " and the total " + "price would be : " + localSum);
	}
}